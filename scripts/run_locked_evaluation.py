...
def main():
    ...
    for idx, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        print(f"[{idx:02d}/{n_test:02d}] {cname:<16} GT={gt:<10} Pred={pred:<10} {sym:<2} ({res['latency_sec']}s) | {res['decision_path']}")
    ...
    print(f"Accuracy: {metrics['accuracy']}% | Precision: {metrics['precision']}% | Recall: {metrics['recall']}% | Specificity: {metrics['specificity']}% | F1: {metrics['f1_score']}%")
