...
CURB_LEFT, APPROACHING, COMMITTED, ON_ROAD, EXITING, CURB_RIGHT = range(6)

def build_temporal_states(x: np.ndarray, min_x: float, max_x: float) -> np.ndarray:
    ...
    direction = 1 if float(x[-1]) > float(x[0]) else -1
    buffer_w = float(max_x - min_x) * 0.15
    committed = False
    s = 0
    for i in range(x.size):
        ...
        s = bi
        if bi == 0:
            if not committed and direction == 1 and xi >= float(min_x) - buffer_w:
                states[i] = APPROACHING
            else:
                states[i] = CURB_LEFT
        elif bi == 1:
            if not committed:
            ...
            if committed and direction == 1 and xi <= float(max_x) + buffer_w:
                states[i] = EXITING
            else:
                states[i] = CURB_RIGHT
    return states
...
class Detection:
    ...
    def pedestrian_crossing(self, dataframe: pl.DataFrame, video_id: str, df_mapping, min_x: float, max_x: float,
        ...
        def build_temporal_states(x: np.ndarray) -> np.ndarray:
            CURB_LEFT, APPROACHING, COMMITTED, ON_ROAD, EXITING, CURB_RIGHT = 0, 1, 2, 3, 4, 5
            base = build_states(x)
            states = np.empty(x.size, dtype=np.int8)
            if x.size == 0:
                return states
