...
class MetaData:
    ...
    def find_values_with_video_id(self, df: pl.DataFrame, key: str):
        ...
        if len(parts) == 3:
            vid, start_str, fps_str = parts
            fps_val_from_key = int(fps_str)
        elif len(parts) == 2:
            vid, start_str = parts
            fps_val_from_key = None
        else:
            return None
        ...
        for row in df.iter_rows(named=True):
            ...
            iso3 = row.get("iso3")
            fps_list = self._safe_literal_eval(row.get("fps_list"))
            ...
            for video, start_list, end_list, tod_list, vtype_list in zip(
                ...
                for idx, s in enumerate(start_list):
                    ...
                    if fps_val_from_key is not None:
                        fps_ret = fps_val_from_key
                    else:
                        fps_ret = 30
                        if isinstance(fps_list, list) and idx < len(fps_list):
                            val = fps_list[idx]
                            if val is not None:
                                try:
                                    fps_ret = int(val)
