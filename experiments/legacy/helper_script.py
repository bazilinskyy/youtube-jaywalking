...
class Youtube_Helper:
    ...
    @staticmethod
    def merge_txt_to_csv_dynamically(txt_location, output_csv, frame_count, zebra_present=False, traffic_light_states=None, pose_data=None, pedestrian_on_zebra=None):
        if traffic_light_states is None:
            traffic_light_states = {}
        if pose_data is None:
            pose_data = {}
        if pedestrian_on_zebra is None:
            pedestrian_on_zebra = {}

        # Define the path for the new text file
        new_txt_file_name = os.path.join(txt_location, f"label_{frame_count}.txt")

        if not os.path.exists(new_txt_file_name) or os.path.getsize(new_txt_file_name) == 0:
            return
        ...
        # Add the new columns
        df['zebra_crossing'] = zebra_present
        df['traffic_light_state'] = 'UNKNOWN'
        df['facing_direction'] = 'UNKNOWN'
        df['stride_ratio'] = 0.0
        ...
        for index, row in df.iterrows():
            ...
            if yid == 9 and uid in traffic_light_states:
                df.at[index, 'traffic_light_state'] = traffic_light_states[uid]
            elif yid == 0:
                if uid in pose_data:
                    df.at[index, 'facing_direction'] = pose_data[uid][0]
                    df.at[index, 'stride_ratio'] = pose_data[uid][1]
                if uid in pedestrian_on_zebra:
                    df.at[index, 'zebra_crossing'] = pedestrian_on_zebra[uid]
    ...
    def prediction_mode(self):
        model = YOLO(self.model).to(0 if torch.cuda.is_available() else "cpu")
    ...
    def tracking_mode(self, input_video_path, output_video_path, video_fps=25):
        ...
        # Always write output video to output_video_path
        display_video_writer = cv2.VideoWriter(output_video_path,
        ...
        while cap.isOpened():
            ...
            if success:
                ...
                if boxes.size(0) == 0:
                    ...
                    # Write label file from results (YOLO save_txt may not work with track)
                    xyxy = results[0].boxes.xyxy.cpu().numpy()
                    cls = results[0].boxes.cls.cpu().numpy()
                    confs = results[0].boxes.conf.cpu().numpy() if results[0].boxes.conf is not None else [1.0] * len(cls)
                    track_ids = results[0].boxes.id.int().cpu().tolist() if results[0].boxes.id is not None else [-1] * len(cls)
                    h, w = frame.shape[:2]
                    with open(text_filename, 'w') as file:
                        for i, (box, c, conf) in enumerate(zip(xyxy, cls, confs)):
                            ...
                            bw = (x2 - x1) / w
                            bh = (y2 - y1) / h
                            file.write(f"{int(c)} {cx:.6f} {cy:.6f} {bw:.6f} {bh:.6f} {int(track_ids[i])}\n")

                # Detect zebra crossing and get its polygon boundary
                zebra_polygon = ZebraDetector.get_zebra_polygon(frame)
                zebra_present = zebra_polygon is not None and len(zebra_polygon) > 0

                # Run custom classifiers (traffic lights and poses)
                traffic_light_states = {}
                pose_data = {}
                pedestrian_on_zebra = {}
                ...
                if results[0].boxes is not None and results[0].boxes.xyxy is not None:
                    ...
                    if results[0].boxes.id is not None:
                        ...
                        track_ids_list = [-1] * len(classes)

                    for idx, (box_xyxy, cls, track_id) in enumerate(zip(xyxy_boxes, classes, track_ids_list)):
                        x1, y1, x2, y2 = map(int, box_xyxy)
                        if cls == 9:  # traffic light
                            tl_crop = frame[max(0, y1):min(frame_height, y2), max(0, x1):min(frame_width, x2)]
                            tl_state = TrafficLightClassifier.classify_state(tl_crop, track_id=track_id)
                            if track_id != -1:
                                traffic_light_states[track_id] = tl_state
                        elif cls == 0:  # person
                            p_crop = frame[max(0, y1):min(frame_height, y2), max(0, x1):min(frame_width, x2)]
                            keypoints, facing, stride = pose_estimator.estimate_pose(p_crop)
                            if track_id != -1:
                                pose_data[track_id] = (facing, stride, keypoints)
                                # Check if bottom-center of the pedestrian is on the zebra crossing
                                p_x_center = int((x1 + x2) / 2)
                                p_y_bottom = int(y2)
                                on_zebra = ZebraDetector.is_point_in_zebra(zebra_polygon, (p_x_center, p_y_bottom))
                                pedestrian_on_zebra[track_id] = on_zebra
                ...
                # Draw Custom Overlay annotations on the annotated_frame
                # 1. Zebra crossing presence & polygon boundary
                zebra_text = f"Zebra Crossing: {'YES' if zebra_present else 'NO'}"
                cv2.putText(annotated_frame, zebra_text, (30, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if zebra_present else (0, 0, 255), 2, cv2.LINE_AA)
                if zebra_polygon is not None and len(zebra_polygon) > 0:
                    pts_poly = np.array(zebra_polygon, dtype=np.int32).reshape((-1, 1, 2))
                    cv2.polylines(annotated_frame, [pts_poly], isClosed=True, color=(0, 255, 0), thickness=2)

                # 2. Traffic Light states, Pedestrian skeletons and crossing safety status
                if results[0].boxes is not None and results[0].boxes.xyxy is not None:
                    xyxy_boxes = results[0].boxes.xyxy.cpu().numpy()
                    classes = results[0].boxes.cls.cpu().numpy()
                    track_ids_list = results[0].boxes.id.int().cpu().tolist() if results[0].boxes.id is not None else [-1] * len(classes)

                    for box_xyxy, cls, track_id in zip(xyxy_boxes, classes, track_ids_list):
                        x1, y1, x2, y2 = map(int, box_xyxy)
                        if cls == 9 and track_id in traffic_light_states:
                            state = traffic_light_states[track_id]
                            # Choose color for text: RED -> red, GREEN -> green, YELLOW -> yellow
                            color = (0, 0, 255) if state == "RED" else (0, 255, 0) if state == "GREEN" else (0, 255, 255) if state == "YELLOW" else (255, 255, 255)
                            cv2.putText(annotated_frame, f"State: {state}", (x1, max(15, y1 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2, cv2.LINE_AA)
                        elif cls == 0 and track_id in pose_data:
                            facing, stride, keypoints = pose_data[track_id]
                            cv2.putText(annotated_frame, f"Pose: {facing} | Stride: {stride:.2f}", (x1, max(15, y1 - 25)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1, cv2.LINE_AA)
                            
                            # Draw keypoints on the crop bounding box (adjusted to absolute coords)
                            for kp in keypoints:
                                kpx, kpy, kpconf = kp
                                if kpconf > 0.4:
                                    abs_x = int(x1 + kpx)
                                    abs_y = int(y1 + kpy)
                                    cv2.circle(annotated_frame, (abs_x, abs_y), 3, (0, 255, 255), -1)

                            # Determine real-time crossing safety / jaywalking status
                            x_center_rel = ((x1 + x2) / 2) / frame_width
                            # Crossing road band check (0.35 to 0.65 relative horizontal positions)
                            if 0.35 <= x_center_rel <= 0.65:
                                on_zebra = pedestrian_on_zebra.get(track_id, False)
                                is_red = any(state == "RED" for state in traffic_light_states.values())
                                facing_dir = pose_data.get(track_id, ("UNKNOWN", 0.0, []))[0]
                                
                                if is_red:
                                    status_text = f"JAYWALKING: RED SIGNAL [{facing_dir}]"
                                    text_color = (0, 0, 255)
                                elif zebra_present and not on_zebra:
                                    status_text = f"JAYWALKING: NO CROSSWALK [{facing_dir}]"
                                    text_color = (0, 0, 255)
                                else:
                                    status_text = f"SAFE CROSSING [{facing_dir}]"
                                    text_color = (0, 255, 0)
                                    
                                cv2.putText(annotated_frame, status_text, (x1, max(15, y1 - 45)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 2, cv2.LINE_AA)
                ...
                Youtube_Helper.merge_txt_to_csv_dynamically(labels_path, output_csv_path, frame_count, zebra_present=zebra_present, traffic_light_states=traffic_light_states, pose_data=pose_data, pedestrian_on_zebra=pedestrian_on_zebra)
                ...
                if delete_frames is False:
                    ...
                    if os.path.exists(image_filename):
                        ...
                        cv2.imwrite(new_img_file_name, annotated_frame)
