...
import polars as pl
from utils.crossing.detection import Detection, build_temporal_states
            ...
            for file in tqdm(os.listdir(folder_path)):
                if file.endswith(".csv"):
                    ...
                    try:
                        ...
                        matched_row = df_mapping[df_mapping["videos"].apply(lambda x: video_id in [v.strip() for v in str(x).strip("[]").split(',')])]
        ...
        for key, df in tqdm(data.items(), total=len(data)):
            ...
            if result is not None:
                ...
                for id, time in df.items():
                    ...
                    # Fusion classification using classify_crossing
                    is_jaywalking = False
                    reason = "NONE"

                    temporal_states = None
                    if 'x-center' in value.columns:
                        ped_data = value.loc[first_occurrence:last_occurrence]
                        x_vals = ped_data['x-center'].values.astype(float)
                        min_x = x_vals.min()
                        max_x = x_vals.max()
                        if max_x > min_x:
                            road_width = max_x - min_x
                            road_min = min_x - road_width * 0.2
                            road_max = max_x + road_width * 0.2
                            temporal_states = build_temporal_states(x_vals, road_min, road_max)

                        light_frame_map = {}
                        if 'traffic_light_state' in ped_data.columns:
                            for idx in ped_data.index:
                                state = value.at[idx, 'traffic_light_state']
                                if state in ("RED", "GREEN", "YELLOW"):
                                    light_frame_map[idx] = state

                        has_zebra_in_video = any(value['zebra_crossing'] == True) if 'zebra_crossing' in value.columns else False
                        on_zebra = any(ped_data['zebra_crossing'] == True) if 'zebra_crossing' in ped_data.columns else False
                        zebra_result = has_zebra_in_video and on_zebra

                        stride_series = []
                        if 'stride_ratio' in ped_data.columns:
                            stride_series = ped_data['stride_ratio'].fillna(0.0).tolist()
                        on_road_mask = [True] * len(ped_data)
                        fps_val = float(fps) if fps else 30.0
                        hesitated = detect_hesitation(stride_series, on_road_mask, fps_val)

                        entry_facing = ped_data['facing_direction'].iloc[0] if 'facing_direction' in ped_data.columns else "UNKNOWN"
                        inattentive = detect_inattentive_entry(str(entry_facing))

                        fusion = classify_crossing(
                            light_states=light_frame_map,
                            zebra_result=zebra_result,
                            fps=fps_val,
                            hesitated=hesitated,
                            inattentive_entry=inattentive,
                            temporal_states=temporal_states,
                            frame_count_start=first_occurrence,
                        )

                        is_jaywalking = fusion["violation"]
                        reason = fusion["violation_type"] or "NONE"
                    else:
                        if 'traffic_light_state' in value.columns:
                            tl_states_crossing = value.loc[first_occurrence:last_occurrence, 'traffic_light_state']
                            if any(tl_states_crossing == 'RED'):
                                is_jaywalking = True
                                reason = "SIGNAL_VIOLATION"

                        if 'zebra_crossing' in value.columns and not is_jaywalking:
                            has_any_zebra_in_video = any(value['zebra_crossing'] == True)
                            if has_any_zebra_in_video:
                                on_zebra = any(value.loc[first_occurrence:last_occurrence, 'zebra_crossing'] == True)
                                if not on_zebra:
                                    is_jaywalking = True
                                    reason = "NO_CROSSWALK"

                    if yolo_id_9_exists:
                        # Save video of event to location with YOLO annotations overlay
                        video_id, start_index = key.rsplit("_", 1)  # split to extract id and index
                        helper.save_event_video(video_id, time, yolo_overlay=True)
                        
                        if is_jaywalking:
                            logger.info(f"Pedestrian {id} in video {video_id} flagged for JAYWALKING: {reason}.")
                        else:
                            logger.info(f"Pedestrian {id} in video {video_id} crossed SAFELY.")
...
if __name__ == "__main__":
    ...
    if os.path.exists(file_results) and not common.get_configs('always_analyse'):
        ...
        with open(file_results, 'rb') as file:
            (data, person_counter, bicycle_counter, car_counter, motorcycle_counter,
             ...
             df_mapping, with_trf_light, without_trf_light, with_trf_light_norm,
             without_trf_light_norm) = pickle.load(file)
        ...
        df_mapping_pl = pl.from_pandas(df_mapping).rename({"city": "locality", "population_city": "population_locality"})
        ...
        for key, value in tqdm(dfs.items(), total=len(dfs)):
            ...
            # Convert to Polars and rename column casings
            pl_df = pl.from_pandas(dfs[key]).rename({
                "YOLO_id": "yolo-id",
                "X-center": "x-center",
                "Y-center": "y-center",
                "Width": "width",
            ...
            # Call advanced Detection
            pedestrian_ids, crossed_ids = detection_instance.pedestrian_crossing(
                pl_df, key, df_mapping_pl, 0.45, 0.55, 0
            )

            # Saving it in a dictionary in: {name_time: count, ids}
            pedestrian_crossing_count[key] = {"count": len(pedestrian_ids), "ids": pedestrian_ids}

            # Saves the time to cross in form {name_time: {id(s): time(s)}}
            data[key] = metrics_instance.time_to_cross(pl_df, pedestrian_crossing_count[key]["ids"], key, df_mapping_pl)
    ...
    # nonzero_without_trf_light_night = df_countries[df_countries["without_trf_light_night"] > 0]

    # max_without_trf_light_night_idx = df_countries["speed_crossing_avg"].idxmax()
    # min_speed_idx = without_trf_day["speed_crossing_avg"].idxmin()

    # max_time_idx = df_countries["time_crossing_avg"].idxmax()
    # min_time_idx = nonzero_without_trf_light_night["time_crossing_avg"].idxmin()

    # # Mean and standard deviation
    # speed_mean = without_trf_day["speed_crossing_avg"].mean()
    # speed_std = without_trf_day["speed_crossing_avg"].std()

    # time_mean = nonzero_without_trf_light_night["time_crossing_avg"].mean()
    # time_std = nonzero_without_trf_light_night["time_crossing_avg"].std()
    ...
    # logger.info(f"Country with the lowest non-zero average speed while crossing: {without_trf_day.loc[min_speed_idx, 'country']} "  # noqa:E501
    #             f"({without_trf_day.loc[min_speed_idx, 'speed_crossing_avg']:.2f})")
    ...
    # logger.info(f"Country with the lowest non-zero average crossing time: {nonzero_without_trf_light_night.loc[min_time_idx, 'country']} "  # noqa: E501
    #             f"({nonzero_without_trf_light_night.loc[min_time_idx, 'time_crossing_avg']:.2f})")
